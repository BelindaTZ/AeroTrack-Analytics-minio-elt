...
async def validar_ejecutar(request: Request):
    ...
    audit.registrar(user["sub"], user["email"], "validar", "modelo_dimensional",
...
async def nuevo_form(request: Request, tabla: str):
    ...
    return render(request, "modelo_dimensional/form_registro.html", {
        "tabla": tabla, "tabla_info": TABLAS[tabla],
        "modo": "crear", "columnas": columnas, "dtypes": dtypes, "pk": pk,
        "registro": {}, "error": None,
    })
...
async def nuevo_guardar(request: Request, tabla: str):
    ...
    audit.registrar(user["sub"], user["email"], "crear", "modelo_dimensional",
                    recurso_tipo=tabla, recurso_id=str(nuevo.get(TABLAS[tabla]["pk"], "")))
...
async def ver_registro(request: Request, tabla: str, pk_val: str):
    ...
    return render(request, "modelo_dimensional/detalle_registro.html", {
        "tabla": tabla, "tabla_info": TABLAS[tabla], "pk": pk, "pk_val": pk_val, "registro": registro,
    })
...
async def editar_form(request: Request, tabla: str, pk_val: str):
    ...
    return render(request, "modelo_dimensional/form_registro.html", {
        "tabla": tabla, "tabla_info": TABLAS[tabla],
        "modo": "editar", "columnas": columnas, "dtypes": dtypes,
        "pk": pk, "pk_val": pk_val, "registro": registro, "error": None,
    })
...
async def editar_guardar(request: Request, tabla: str, pk_val: str):
    ...
    try:
        ...
        return _tmpl_error(request, str(exc), 500)
    audit.registrar(user["sub"], user["email"], "editar", "modelo_dimensional",
                    recurso_tipo=tabla, recurso_id=pk_val)
...
async def eliminar(request: Request, tabla: str, pk_val: str):
    ...
    try:
        ...
        return _tmpl_error(request, str(exc), 500)
    audit.registrar(user["sub"], user["email"], "eliminar", "modelo_dimensional",
                    recurso_tipo=tabla, recurso_id=pk_val)
...
async def lista_registros(request: Request, tabla: str, page: int = 1, q: str = ""):
    ...
    return render(request, "modelo_dimensional/lista_registros.html", {
        "tabla": tabla,
