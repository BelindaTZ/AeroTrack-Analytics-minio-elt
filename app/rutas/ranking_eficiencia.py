...
_AIRPORTS: dict[str, tuple[float, float]] = {
    ...
    "FAI": (64.8151, -147.8562),
    "JNU": (58.3549, -134.5763),
    "KOA": (19.7388, -156.0456),
    "LIH": (21.9760, -159.3390),
    "ITO": (19.7204, -155.0485),
    "DSM": (41.5340, -93.6631),
    "GRR": (42.8808, -85.5228),
    "FSD": (43.5820, -96.7419),
    "BZN": (45.7774, -111.1533),
    ...
    "SBA": (34.4262, -119.8403),
    "SBP": (35.2368, -120.6424),
    "MRY": (36.5870, -121.8429),
    "LGB": (33.8177, -118.1516),
    "TWF": (42.4818, -114.4877),
    "LWS": (46.3745, -117.0153),
    "PIH": (42.9098, -112.5958),
    "IDA": (43.5146, -112.0702),
    "CPR": (42.9080, -106.4644),
    "RAP": (44.0453, -103.0574),
    "BIS": (46.7727, -100.7466),
    "FAR": (46.9207, -96.8158),
    "GFK": (47.9493, -97.1762),
    "MOT": (48.2594, -101.2797),
    "MSN": (43.1399, -89.3375),
    "GRB": (44.4851, -88.1297),
    "ATW": (44.2571, -88.5191),
    "RST": (43.9088, -92.5001),
    ...
    "LBB": (33.6636, -101.8229),
    "SPS": (33.9888, -98.4919),
    "ROW": (33.3016, -104.5306),
    "TUS": (32.1161, -110.9410),
    "FLG": (35.1385, -111.6703),
    "YUM": (32.6566, -114.6060),
    "ELY": (39.2997, -114.8421),
    "BDL": (41.9389, -72.6831),
    "HPN": (41.0670, -73.7076),
    "ALB": (42.7483, -73.8017),
    "SWF": (41.5041, -74.1048),
    ...
    "PLN": (45.5708, -84.7967),
    "CIU": (46.2508, -84.4728),
    "ISN": (48.1779, -103.6419),
    "DIK": (46.7974, -102.8019),
    "GCC": (44.3489, -105.5396),
    "SHR": (44.7692, -106.9802),
    "RKS": (41.5942, -109.0658),
    "CYS": (41.1558, -104.8119),
    "LAR": (41.3121, -105.675),
    "CDC": (37.7010, -113.0987),
    "SGU": (37.0363, -113.5103),
    "VEL": (40.4408, -109.5099),
    "BKG": (36.6819, -93.2003),
    "XWA": (48.2594, -101.2797),
    "DBQ": (42.4020, -90.7095),
    "ALO": (42.5571, -92.4003),
    "COU": (38.8181, -92.2196),
    "SWO": (36.1771, -97.0860),
    ...
    "HLN": (46.6068, -111.9833),
    "BIL": (45.8077, -108.5428),
    "CDV": (60.4919, -145.4778),
    "SIT": (57.0471, -135.3615),
    "KTN": (55.3556, -131.7137),
    "BET": (60.7798, -161.8380),
    "OME": (64.5122, -165.4451),
    "OTZ": (66.8847, -162.5985),
    "ADQ": (57.7500, -152.4938),
    "DLG": (59.0445, -158.5067),
    "SCC": (70.1947, -148.4651),
}
...
def _mapa_rutas(rows: list[dict]) -> str:
    ...
    fig.update_geos(
        scope="usa",
        showland=True,
        landcolor="rgba(22,27,40,1)",
        showocean=True,
        oceancolor="rgba(10,13,20,1)",
        showlakes=True,
        lakecolor="rgba(10,13,20,1)",
        showcoastlines=True,
        coastlinecolor="rgba(100,116,139,0.35)",
        showsubunits=True,
        subunitcolor="rgba(100,116,139,0.12)",
        showcountries=True,
...
def _calcular_ranking_agg(df_rutas: pd.DataFrame, umbral: float) -> list[dict]:
    ...
    grp["retraso_prom"] = grp["retraso_prom"].apply(safe_float).round(1)
...
def _detalle_ruta(df: pd.DataFrame, origen: str, dest: str) -> dict:
    ...
    if "Month" in vuelos_op.columns and "ArrDel15" in vuelos_op.columns:
        grp = (
            ...
            .agg(t=("pk_vuelo", "count"), ok=("ArrDel15", lambda x: (x == 0).sum()))
            .reset_index()
        )
...
def ranking(request: Request, year: str = "", month: str = "", airline: str = ""):
    ...
    return render(
        ...
        {
            "rows": rows,
            "grafico_scatter": grafico_scatter,
        ...
        if tipo == "ruta" and ruta_val:
            ...
            if row:
                ...
                ctx["Estado eficiencia"] = (
                    f"INEFICIENTE (índice > {1 + umbral:.2f})"
                    if row["ineficiente"]
            ...
            if ineficientes:
                ...
                ctx["Ruta más ineficiente"] = (
                    f"{peor['ruta']} (índice {peor['eficiencia_media']:.3f}, retraso {peor['retraso_prom']:.1f} min)"
                )
            eficientes = [r for r in rows if not r["ineficiente"]]
            if eficientes:
