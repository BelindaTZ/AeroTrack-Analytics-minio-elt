...
from app.shared.analytics import get_aerolinas, load_agg, load_enriched_fact
...
_AIRLINE_CODES: list[str] | None = None
...
def parse_intent(question: str) -> dict:
    ...
    if "ruta" not in filtros:
        ...
        airport_matches = [a for a in airport_matches if a != filtros.get("airline", "") and a not in _NO_IATA]
...
def _detect_question_type(question: str) -> set:
    ...
    if any(w in q for w in ["desvío", "desvio", "alternativo", "divert"]):
        tipos.add("desvio")
    if any(w in q for w in ["tendencia", "mensual", "por mes", "evolución", "evolucion", "cada mes", "mes a mes"]):
        tipos.add("tendencia")
    if any(
        w in q
        for w in [
...
def _seccion_kpis_globales(df: pd.DataFrame) -> str:
    ...
    if "delay_avg" in df.columns and op > 0:
        delay = safe_float((df["delay_avg"] * df["total_vuelos"]).sum() / op)
    return (
        "=== KPIs GLOBALES ===\n"
...
def _seccion_ranking_aerolineas(df: pd.DataFrame) -> str:
    ...
    if "delay_avg" in df.columns:
        delay_grp = (
            ...
            .apply(
                lambda x: (
                    safe_float((x["delay_avg"] * x["total_vuelos"]).sum() / x["total_vuelos"].sum())
                    if x["total_vuelos"].sum() > 0
                    else 0.0
                )
            )
            .rename("delay")
...
def _seccion_cancelaciones_causa(df: pd.DataFrame) -> str:
    ...
    lines = ["=== CANCELACIONES POR CÓDIGO FAA ===", f"Total cancelados: {total_canc:,}"]
...
def _seccion_rutas_eficiencia(df: pd.DataFrame) -> str:
    ...
    if has_delay:
        ...
        for _, row in peores.iterrows():
            ...
            lines.append(f"  {row['_ruta']}: {safe_float(row['retraso_prom']):.1f} min retraso{efic_s}{vuelos_s}")
        ...
        for _, row in mejores.iterrows():
            lines.append(f"  {row['_ruta']}: {safe_float(row['retraso_prom']):.1f} min retraso")
    elif has_total:
...
def _seccion_otp_dia_semana(df: pd.DataFrame) -> str:
    ...
    for _, row in df.iterrows():
        dia = (
            _DIAS_LABEL[int(row["day_of_week"])] if 1 <= int(row["day_of_week"]) <= 7 else str(int(row["day_of_week"]))
        )
        lines.append(f"  {dia}: OTP {safe_float(row['otp_pct']):.1f}%")
    return "\n".join(lines)
...
def _seccion_cancelaciones_aerolinea_causa(df: pd.DataFrame, airline: str | None = None) -> str:
    ...
    if airline:
        ...
        if total == 0:
            return ""
        lines = [f"=== CANCELACIONES {airline} POR CÓDIGO FAA ===", f"Total cancelados: {total:,}"]
...
def _seccion_otp_aerolinea_dia_semana(df: pd.DataFrame, airline: str | None = None, dow: str | None = None) -> str:
    ...
    if airline:
        ...
        for _, row in sub.iterrows():
            dia = (
                _DIAS_LABEL[int(row["day_of_week"])]
                if 1 <= int(row["day_of_week"]) <= 7
                else str(int(row["day_of_week"]))
            )
            lines.append(f"  {dia}: OTP {safe_float(row['otp_pct']):.1f}%  ({int(row['total_vuelos']):,} vuelos)")
        return "\n".join(lines)
    ...
    if dow:
        ...
        for i, (_, row) in enumerate(sub.iterrows()):
            lines.append(f"  #{i + 1} {row['carrier']}: OTP {safe_float(row['otp_pct']):.1f}%")
        return "\n".join(lines)
...
def _seccion_dinamica_aerolinea(filtros: dict, tipos: set) -> str:
    ...
    try:
        ...
        if avail and "delay_cause" in tipos:
            ...
            totals = {l: float(pd.to_numeric(op[c], errors="coerce").fillna(0).sum()) for c, l in avail.items()}
            grand = sum(totals.values())
            if grand > 0:
        ...
        if "dia_semana" in tipos and "DayOfWeek" in df.columns:
            ...
            if "_at" in op.columns:
                ...
                for _, row in dow_grp.sort_values("DayOfWeek").iterrows():
                    dia = (
                        _DIAS_LABEL[int(row["DayOfWeek"])]
                        if 1 <= int(row["DayOfWeek"]) <= 7
                        else str(int(row["DayOfWeek"]))
                    )
...
def _seccion_ruta_especifica(ruta: str) -> str:
    ...
    try:
        ...
        delay = safe_float(op["ArrDelayMinutes"].fillna(0).mean()) if "ArrDelayMinutes" in op.columns else 0.0
        ...
        if "Reporting_Airline" in df.columns:
            ...
            for _, row in al_grp.iterrows():
                lines.append(
                    f"  {row['Reporting_Airline']}: {int(row['total']):,} vuelos, {row['tasa']:.1f}% cancelados"
                )
...
def _seccion_aeropuerto(aeropuerto: str, df_rutas: pd.DataFrame, df_canc_ruta: pd.DataFrame) -> str:
    ...
    if not df_rutas.empty and "origin" in df_rutas.columns:
        ...
        if not como_origen.empty and "retraso_prom" in como_origen.columns:
            delay_orig = safe_float(como_origen["retraso_prom"].mean())
        ...
        if not como_destino.empty and "retraso_prom" in como_destino.columns:
            delay_dest = safe_float(como_destino["retraso_prom"].mean())
    ...
    if not df_canc_ruta.empty and "origin" in df_canc_ruta.columns:
        ap_rows = df_canc_ruta[(df_canc_ruta["origin"] == aeropuerto) | (df_canc_ruta["dest"] == aeropuerto)]
        if not ap_rows.empty and "tasa_cancelacion" in ap_rows.columns:
            tasa_media = safe_float(ap_rows["tasa_cancelacion"].mean()) * 100
...
def build_context(filtros: dict, question: str = "") -> str:
        ...
        try:
            ...
            if df_otp.empty and f_otp:
                ...
                if not df_otp_full.empty:
                    year_avail = (
                        sorted(df_otp_full["year"].unique().astype(int).tolist())
                        if "year" in df_otp_full.columns
                        else []
                    )
    ...
    if filtros.get("aeropuerto") and not filtros.get("ruta"):
        if "rutas_eficiencia" in activas or "cancelaciones_ruta" in activas:
            try:
                df_rutas_ap = (
                    load_agg("agg_rutas_eficiencia", f_mes) if "rutas_eficiencia" in activas else pd.DataFrame()
                )
                df_canc_ap = (
                    load_agg("agg_cancelaciones_ruta", f_global) if "cancelaciones_ruta" in activas else pd.DataFrame()
                )
            except Exception:
    ...
    return result if result else ("No hay datos disponibles. Asegúrese de que el pipeline ELT se ha ejecutado.")
...
def build_messages(history: list[dict], context: str, question: str) -> list[dict]:
    ...
    user_with_ctx = f"=== DATOS DEL SISTEMA ===\n{context}\n=== FIN DE DATOS ===\n\nPregunta: {question}"
