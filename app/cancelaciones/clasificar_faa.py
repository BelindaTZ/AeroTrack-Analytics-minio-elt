...
def _causas_faa_agg(df_canc: pd.DataFrame, df_kpi: pd.DataFrame) -> dict:
    ...
    if not df_canc.empty and "cancellation_code" in df_canc.columns:
        grp = (
            ...
            .groupby("cancellation_code")["total_cancelados"]
            .sum()
            .reset_index()
        )
    ...
    total_cancelados_all = (
        int(df_kpi["total_cancelados"].sum())
        if not df_kpi.empty and "total_cancelados" in df_kpi.columns
        else total_cancelados_faa
    )
    tasa = safe_float(total_cancelados_all / total_vuelos) if total_vuelos > 0 else 0.0
...
def _tendencias_mensuales_agg(df_canc: pd.DataFrame) -> dict:
    ...
    grp["mes_label"] = grp["month"].apply(lambda m: _MESES[int(m) - 1] if 1 <= int(m) <= 12 else str(m))
...
def _desvios_agg(df_dev: pd.DataFrame) -> list[dict]:
    ...
    grp = (
        ...
        .reset_index()
        .sort_values("total_desvios", ascending=False)
        .head(20)
    )
    rows = []
        ...
        rows.append(
            {
                ...
                "div_arr_delay": round(float(safe_float(r.get("divarrdelay_avg", 0) or 0)), 1),
                "div_distance": round(float(safe_float(r.get("divdistance_avg", 0) or 0)), 1),
            }
        )
    return rows
    ...
    try:
        ...
        if tipo == "causa_faa" and code:
            ...
            if idx >= 0 and total_c > 0:
                ...
                otras = [
                    f"{causas_data['labels'][j]} ({causas_data['counts'][j] / total_c * 100:.0f}%)"
                    for j in range(len(causas_data["labels"]))
                    if j != idx
                ]
                if otras:
            ...
            if causas_data["labels"] and total_c > 0:
                ...
                ctx["Causa dominante"] = (
                    f"{causas_data['descriptions'][dom_idx]} ({causas_data['counts'][dom_idx] / total_c * 100:.0f}%)"
                )
