    ...
    for col, label in _CAUSA_LABEL_MAP.items():
        if col in df_causas.columns:
            ...
            if total > 0:
                ...
                result["counts"].append(round(safe_float(total), 0))
    return result
...
def _tendencias_otp_mensual_agg(df_otp: pd.DataFrame, airline: str = "", year: str = "") -> dict:
    ...
    if year:
        ...
        if "month" in df_prev.columns and not df_prev.empty:
            ...
            grp_prev["otp"] = (
                (grp_prev["vuelos_at"] / grp_prev["total"].replace(0, float("nan")) * 100).fillna(0.0).round(1)
            )
    ...
    try:
        ...
        if tipo == "aerolinea" and airline_val:
            ...
            if idx >= 0 and total_c > 0:
                ...
                otras = [
                    f"{causas['labels'][j]} ({causas['counts'][j] / total_c * 100:.0f}%)"
                    for j in range(min(2, len(causas["labels"])))
                    if j != idx
                ]
                if otras:
            ...
            if datos_otp:
                ...
                ctx["Aerolínea con mejor OTP"] = (
                    f"{airline_name(sorted_al[-1]['aerolinea'])} ({sorted_al[-1]['otp']:.1f}%)"
                )
                ctx["Aerolínea con peor OTP"] = (
                    f"{airline_name(sorted_al[0]['aerolinea'])} ({sorted_al[0]['otp']:.1f}%)"
                )
                ctx["Aerolíneas bajo el umbral 80%"] = (
                    f"{sum(1 for r in datos_otp if r['otp'] < 80)} de {len(datos_otp)}"
                )
    ...
    try:
        ...
        if "origin" in df_r.columns and "dest" in df_r.columns:
            rutas_disponibles = sorted(
                (df_r["origin"].astype(str) + "-" + df_r["dest"].astype(str)).dropna().unique().tolist()[:500]
            )
    except Exception:
        pass
