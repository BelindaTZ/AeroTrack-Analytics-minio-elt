...
async def lista(request: Request):
    ...
    for rol in roles:
        rol["_count_usuarios"] = pb_client.list_records_all("app_users", filter=f'rol_id="{rol["id"]}"').__len__()
...
async def crear(request: Request, nombre: str = Form(...), descripcion: str = Form("")):
    ...
    try:
        record = pb_client.create_record(
            "roles", {"nombre": nombre, "descripcion": descripcion, "es_sistema": False, "activo": True}
        )
...
async def eliminar(request: Request, rid: str):
    ...
    audit.registrar(user["sub"], user["email"], "eliminar", "seguridad", recurso_tipo="rol", recurso_id=rid)
