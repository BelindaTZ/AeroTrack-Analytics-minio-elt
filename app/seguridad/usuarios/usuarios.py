...
async def lista(request: Request, page: int = 1, q: str = "", rol_id: str = "", activo: str = ""):
    ...
    usuarios = pb_client.list_records(
        "app_users", filter=filt, expand="rol_id", page=page, per_page=20, sort="-created"
    )
...
async def crear(
    ...
    try:
        record = pb_client.create_record(
            "app_users",
            {
                "nombre": nombre,
                "email": email,
    ...
    if err_email:
        msg += " No se pudo enviar el email de bienvenida — verifique la configuración SMTP."
...
async def reset_password(request: Request, uid: str, nombre: str = Form("")):
    ...
    audit.registrar(user["sub"], user["email"], "editar", "seguridad", recurso_tipo="password_reset", recurso_id=uid)
