...
from app.config import MINIO_ENDPOINT, MINIO_ACCESS_KEY, MINIO_SECRET_KEY, PB_URL, AIRFLOW_URL, AIRFLOW_USER, AIRFLOW_PASSWORD
...
def _check_pocketbase() -> dict:
    ...
    try:
        ...
        return {"nombre": "PocketBase", "ok": ok, "latencia_ms": latencia,
                "estado": "online" if ok else "degradado",
                "url": f"{PB_URL}/_/", "icono": "bi-database-fill"}
    except Exception as exc:
        return {"nombre": "PocketBase", "ok": False, "latencia_ms": -1,
                "estado": "offline", "error": str(exc),
                "url": f"{PB_URL}/_/", "icono": "bi-database-fill"}
...
def _check_minio() -> dict:
    ...
    try:
        ...
        return {
            "nombre": "MinIO", "ok": True, "latencia_ms": latencia,
            "estado": "online", "buckets": bucket_list,
            "metricas": metricas,
            "url": f"http://{host}:9001", "icono": "bi-server",
        }
    except Exception as exc:
        return {"nombre": "MinIO", "ok": False, "latencia_ms": -1,
                "estado": "offline", "error": str(exc),
                "url": "#", "icono": "bi-server"}
...
def _check_airflow() -> dict:
    ...
    try:
        ...
        return {
            "nombre": "Airflow", "ok": ok, "latencia_ms": latencia,
            "estado": "online" if ok else "degradado",
            "scheduler": "healthy" if scheduler_ok else "unknown",
            "url": f"{AIRFLOW_URL}", "icono": "bi-gear-wide-connected",
        }
    except Exception as exc:
        return {"nombre": "Airflow", "ok": False, "latencia_ms": -1,
                "estado": "offline", "error": str(exc),
                "url": f"{AIRFLOW_URL}", "icono": "bi-gear-wide-connected"}
