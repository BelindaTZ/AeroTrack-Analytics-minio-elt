...
from app.shared.analytics import get_aerolinas, get_years, load_agg, load_enriched_fact
...
# Isolation Forest (opcional — fallback a Z-score si no está disponible)
try:
...
def _load_otp_series(airline: str = "", year: str = "") -> pd.DataFrame:
    ...
    grp = df.groupby(group_cols).agg(total=("total_vuelos", "sum"), a_tiempo=("vuelos_a_tiempo", "sum")).reset_index()
...
def _proyectar_otp(df: pd.DataFrame, horizonte: int) -> dict:
    ...
    if "year" in df.columns and "month" in df.columns:
        labels_hist = [f"{_MESES[int(m) - 1]} {int(y)}" for y, m in zip(df["year"], df["month"])]
    ...
    try:
        ...
        fitted = [round(min(100.0, max(0.0, safe_float(v))), 1) for v in mdl.fittedvalues]
    ...
    fc = [min(100.0, max(0.0, safe_float(v))) for v in forecast]
    ...
    return {
        "historico": [round(safe_float(v), 1) for v in otp_vals],
        ...
        for day in range(1, 8):
            for month in range(1, 13):
                ...
                if metric == "otp" and "ArrDel15" in sub.columns:
                    ...
                    val = 0.0
                matrix[day - 1][month - 1] = round(safe_float(val), 1)
...
def _detectar_anomalias(df: pd.DataFrame, airline: str = "") -> list[dict]:
    ...
    for _, row in df_m.iterrows():
        ...
        mes_rows = (
            df_m[df_m["month"] == int(row["month"])].sort_values("year")
            if "year" in df_m.columns
            else df_m[df_m["month"] == int(row["month"])]
        )
...
def _detectar_anomalias_iforest(df: pd.DataFrame, airline: str = "") -> list[dict]:
    """Detecta anomalías usando Isolation Forest (ML). Fallback a Z-score si sklearn no disponible."""
    if not _IFOREST_OK:
        return _detectar_anomalias(df, airline)
    ...
    # Preparar features: OTP + mes (ciclical) + año
    features = df[["otp"]].copy()
    if "month" in df.columns:
        features["month_sin"] = df["month"].apply(lambda m: math.sin(2 * math.pi * m / 12))
        features["month_cos"] = df["month"].apply(lambda m: math.cos(2 * math.pi * m / 12))
    if "year" in df.columns:
        features["year_norm"] = (df["year"] - df["year"].min()) / max(df["year"].max() - df["year"].min(), 1)
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        if preds[i] == -1:  # Anomalía
            ...
            # Sparkline
            mes_rows = (
                df[df["month"] == int(row["month"])].sort_values("year")
                if "year" in df.columns
                else df[df["month"] == int(row["month"])]
            )
            spark_vals = [round(float(v), 1) for v in mes_rows["otp"].tolist()]
            spark_pos = 0
            if "year" in mes_rows.columns:
                for j, yr_val in enumerate(mes_rows["year"].tolist()):
                    if int(yr_val) == yr:
                        spark_pos = j
                        break

            # Calcular delta vs promedio del mes
            mean_m = float(df[df["month"] == int(row["month"])]["otp"].mean())
            delta = round(row["otp"] - mean_m, 1)
            tipo = "bajo" if delta < 0 else "alto"

            anomalias.append({
                "periodo": periodo,
                "otp": round(row["otp"], 1),
                "promedio_mes": round(mean_m, 1),
                "delta": delta,
                "z": round(float(scores[i]), 2),  # Score del modelo (negativo = más anómalo)
                "tipo": tipo,
                "sparkline": _sparkline_svg(spark_vals, spark_pos, tipo),
    ...
    anomalias.sort(key=lambda a: a["z"])
    return anomalias[:5]
...
def _calcular_ranking_riesgo(year: str = "") -> list[dict]:
    ...
    df = df.copy()
    df["otp_row"] = (df["vuelos_a_tiempo"] / df["total_vuelos"].replace(0, float("nan")) * 100).fillna(0.0)

    agg = (
        df.groupby(carrier_col)
        .agg(total=("total_vuelos", "sum"), a_tiempo=("vuelos_a_tiempo", "sum"), n_meses=(carrier_col, "count"))
        .reset_index()
    )
    agg["otp_mean"] = (agg["a_tiempo"] / agg["total"].replace(0, float("nan")) * 100).fillna(0.0)
...
def anomalias_endpoint(request: Request, airline: str = "", year: str = ""):
    _perm_ver(request)
    df_otp = _load_otp_series(airline=airline, year=year)
    # Usar Isolation Forest si está disponible, fallback a Z-score
    anomalias = _detectar_anomalias_iforest(df_otp, airline=airline)
    return JSONResponse({
        "anomalias": anomalias,
        "metodo": "isolation_forest" if _IFOREST_OK else "z_score",
        "iforest_disponible": _IFOREST_OK,
    })
