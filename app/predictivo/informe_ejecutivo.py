...
def _svg_line(labels: list, values: list, width: int = 560, height: int = 180) -> str:
    ...
    path_d = " ".join(f"{'M' if j == 0 else 'L'}{px(i, v)[0]:.1f},{px(i, v)[1]:.1f}" for j, (i, v) in enumerate(pts))
    ...
    circles = "".join(f'<circle cx="{px(i, v)[0]:.1f}" cy="{px(i, v)[1]:.1f}" r="3" fill="#3b82f6"/>' for i, v in pts)
    return (
...
def _html_informe(
    ...
    svg_hmap = _svg_hmap_mini(
        heatmap.get("matrix", []), heatmap.get("rows", _DIAS), heatmap.get("cols", _MESES), heatmap.get("metric", "otp")
    )
    ...
    if proy_v and proy_l:
        rows_proy = "".join(
            ...
            for l, v, lo, hi in zip(proy_l, proy_v, proy.get("ic_inf", []), proy.get("ic_sup", []))
        )
        proy_table = (
...
async def generar_informe(request: Request):
    ...
    try:
        ...
        audit.registrar(
            ...
            "exportar",
            "predictivo",
            recurso_tipo="informe_ejecutivo",
            recurso_id=obj_name,
        )
