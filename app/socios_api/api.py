...
def _validar_api_key(request: Request) -> tuple[Optional[dict], Optional[JSONResponse]]:
    token = request.headers.get("Authorization", "").replace("Bearer ", "").strip()
    if not token:
        token = request.query_params.get("api_key", "")
    if not token:
        return None, JSONResponse({"error": "API key requerida. Usa header Authorization: Bearer <key> o query param api_key."}, status_code=401)
    ...
    try:
        pb.update_record("pb_api_claves", record["id"], {
            "consultas_hoy": consultas_hoy + 1,
            "consultas_total": record.get("consultas_total", 0) + 1,
            "ultima_consulta": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.000Z"),
        })
    except Exception:
        pass
...
def _registrar_llamada(clave_id: str, endpoint: str, metodo: str,
                       codigo: int, tiempo_ms: int, ip: str) -> None:
    try:
        pb.create_record("pb_api_historial", {
            "clave_id":        clave_id,
            "endpoint":        endpoint,
            "metodo":          metodo,
...
async def api_otp(request: Request, airline: str = "", year: str = "", month: str = ""):
    ...
    t0 = time.time()
    try:
        df = load_agg("agg_otp_aerolinea_mes", {k: v for k, v in {
            "airline": airline, "year": year, "month": month,
        }.items() if v})
        result = _safe_df(df)
        codigo = 200
        ...
        codigo = 500
    _registrar_llamada(clave["id"], "/api/v1/otp", "GET", codigo,
                       int((time.time() - t0) * 1000), _client_ip(request))
    return JSONResponse({"ok": codigo == 200, "data": result, "total": len(result) if isinstance(result, list) else 0}, status_code=codigo)
...
async def api_rutas(request: Request, year: str = "", top_n: int = 10):
    ...
    try:
        ...
        result = {"error": str(exc)}
        codigo = 500
    _registrar_llamada(clave["id"], "/api/v1/rutas", "GET", codigo,
                       int((time.time() - t0) * 1000), _client_ip(request))
    return JSONResponse({"ok": codigo == 200, "data": result, "total": len(result) if isinstance(result, list) else 0}, status_code=codigo)
...
async def api_cancelaciones(request: Request, year: str = "", month: str = ""):
    ...
    t0 = time.time()
    try:
        df = load_agg("agg_cancelaciones_causa", {k: v for k, v in {
        ...
        result = {"error": str(exc)}
        codigo = 500
    _registrar_llamada(clave["id"], "/api/v1/cancelaciones", "GET", codigo,
                       int((time.time() - t0) * 1000), _client_ip(request))
    return JSONResponse({"ok": codigo == 200, "data": result, "total": len(result) if isinstance(result, list) else 0}, status_code=codigo)
...
async def api_kpis(request: Request, year: str = "", month: str = ""):
    ...
    t0 = time.time()
    try:
        df = load_agg("agg_kpi_global_dia", {k: v for k, v in {
            "year": year, "month": month,
        }.items() if v})
        result = _safe_df(df)
        codigo = 200
        ...
        result = {"error": str(exc)}
        codigo = 500
    _registrar_llamada(clave["id"], "/api/v1/kpis", "GET", codigo,
                       int((time.time() - t0) * 1000), _client_ip(request))
    return JSONResponse({"ok": codigo == 200, "data": result, "total": len(result) if isinstance(result, list) else 0}, status_code=codigo)
