...
def extract_pipeline() -> None:
    ...
    de forma incremental (sin OOM Kill) y los sube a MinIO en
    aerotrack-raw/vuelos_raw.parquet."""
    ...
    print(f"[EXTRACT] ✅ {registros_ok:,} registros extraídos")
 
    # ── 6. Subir Parquet a MinIO ──────────────────────────────
...
def transform_pipeline() -> None:
    ...
    def key_str(df, cols):
        # Concatenación vectorizada — evita loop Python fila a fila (50x más rápido)
    ...
    # ── Fact table con map() ─────────────────────────────────────
    # IMPORTANTE: se reutiliza df en lugar de copiarlo para evitar OOM Kill.
    # Se eliminan las columnas innecesarias in-place (0 bytes extra).
    print("\n[TRANSFORM] Generando fact_vuelo con map()...", flush=True)

    import gc

    metricas    = cols_ok(df, ["AirTime","Distance","Flights","ActualElapsedTime","CRSElapsedTime","TaxiIn","TaxiOut"])
    cols_needed = set(cols_ok(df, [
        "FlightDate","Reporting_Airline","Origin","Dest","Tail_Number",
        "DistanceGroup","CRSDepTime","Cancelled","CancellationCode","Diverted",
    ...
    # Eliminar columnas innecesarias IN-PLACE — sin copia, sin pico de RAM
    cols_to_drop = [c for c in df.columns if c not in cols_needed]
    df.drop(columns=cols_to_drop, inplace=True)
    gc.collect()
    fact = df   # renombrar — mismo objeto, 0 bytes extra
    del df

    print(f"  Columnas en fact: {len(fact.columns)} | Filas: {len(fact):,}", flush=True)

    # Construir todos los lookup dicts ANTES de asignar FKs y liberar cada
    # dim_* inmediatamente — evita tener fact (2M filas) + todas las dims
    # en RAM al mismo tiempo, lo que causaba OOM Kill.
    print("  Construyendo lookups y liberando dims...", flush=True)
    ...
    cols_rc_fact = cols_ok(fact, cols_rc) if cols_rc else []
    lookup_rc = dict(zip(key_str(dim_retraso[cols_rc_fact], cols_rc_fact), dim_retraso["pk_retraso_causa"])) if cols_rc_fact else {}
    del dim_retraso

    cols_dev_key = cols_ok(fact, ["DivAirportLandings", "Div1Airport"])
    lookup_desvio = dict(zip(key_str(dim_desvio[cols_dev_key], cols_dev_key), dim_desvio["pk_desvio"])) if cols_dev_key else {}
    del dim_desvio

    gc.collect()
    print("  Lookups listos — asignando FKs...", flush=True)

    # Categorical dtype rompe .map().fillna(0) — convertir a object en bloque
    cat_cols = [c for c in fact.select_dtypes(include="category").columns]
    if cat_cols:
        fact[cat_cols] = fact[cat_cols].astype(object)
        gc.collect()

    # int32 usa 50% menos RAM que int64 para columnas FK
    ...
    if lookup_cl:
        fact["fk_clasificacion_retraso"] = key_str(fact, cols_cl).map(lookup_cl).fillna(0).astype("int32")
    del lookup_cl

    if lookup_rc:
        fact["fk_retraso_causa"] = key_str(fact, cols_rc_fact).map(lookup_rc).fillna(0).astype("int32")
    del lookup_rc
    print("  ✓ fk_clasificacion_retraso, fk_retraso_causa", flush=True)

    if lookup_desvio:
        fact["fk_desvio"] = key_str(fact, cols_dev_key).map(lookup_desvio).fillna(0).astype("int32")
    del lookup_desvio
    gc.collect()
    print("  ✓ fk_desvio — Todos los FKs asignados", flush=True)

    # Construir fact_vuelo final — solo FKs + métricas
    fks        = [c for c in fact.columns if c.startswith("fk_")]
    metricas_f = cols_ok(fact, metricas)
