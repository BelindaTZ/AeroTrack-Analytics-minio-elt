...
# ── URLs según contexto ───────────────────────────────────────
MINIO_ENDPOINT = (
    os.getenv("MINIO_URL_DOCKER", "minio:9000")
    if IN_DOCKER
    else os.getenv("MINIO_URL", "localhost:9000")
)
PB_BASE_URL = (
    os.getenv("PB_URL_DOCKER", "http://pocketbase:8090")
    if IN_DOCKER
    else os.getenv("PB_URL", "http://localhost:8090")
)
CSV_FILE_PATH = (
